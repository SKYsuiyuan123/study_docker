...
技术原理
Docker  利用了 Linux 内核的两大原生功能 Cgroups 和 Namespaces 实现容器化。
Cgroups 用来限制和隔离进程的资源利用，可以为每个容器设定 CPU，内存，网络带宽等资源的使用上限，确保了一个容器的资源消耗不会影响到宿主机，或者其它正在运行的容器。
Namespaces 用来隔离进程的资源视图，它使得容器只能看到自己内部的进程ID 网络资源和文件目录 而看不到宿主机的。所以容器本质上还是一个特殊的进程。不过当我们进入到容器内部后，容器内部看起来就像一个独立的操作系统。
...
```shell
# 查看 docker Images, Containers, Volumes 的信息
$ docker system df

# 查看启动的容器 运行资源占用情况 (是实时展现的，但是没有历史记录，没有预警监控。)
$ docker stats

# 监控三件套：CAdvisor 监控收集 + InfluxDB 存储数据 + Granfana 展示图表
```
...
# 拉取指定架构的 镜像，默认会拉取最适合当前宿主机 CPU 架构的镜像。
$ docker pull --platform=xxx nginx # linux/amd64, linux/arm64, linux/arm/v7, linux/arm/v6
```
...
每个容器 都是一个独立的运行环境，每个容器内部表现的都像一个独立的 Linux 系统。
...
$ docker exec -it tomcat-1 /bin/sh
...
# --restart=always: 指定 docker 启动后 容器也跟着启动。
# --restart=unless-stopped: 手动停止的容器，docker 启动后不会重启。
# --rm: 当容器停止时，自动删除容器。
# -P: 随机指定端口。(一般不用)
# run 时，本地没有该镜像，则会 去远程拉取该镜像，然后执行。

$ docker run -d -p 8081:8080 --name tomcat-1 tomcat

# 创建容器 但是不立即启动
$ docker create -p 8082:8080 --name tomcat-2 tomcat

# 修改 restart 模式
$ docker update --restart=no tomcat-1
```
...
docker 内启动的容器都有独立的 IP，但是这个 IP 是会变动的。
...
`bridge`: 默认网桥 (docker0)，普通容器默认都连它。(每一个容器内部都叫 eth0, docker 内部都叫 veth，一一匹配。)

`host`: 直接用宿主机网络栈。(容器将不会获得一个独立的 Network Namespace，而是和宿主机共用一个 Network Namespace。容器将不会虚拟出自己的网卡而是使用宿主机的IP 和 端口。) (不再需要 -p 网络映射)

`none`: 无网络，只有回环口。容器没有网卡、IP、路由等信息，需要自己为 docker 容器添加网卡、配置IP 等。

`container`: 新创建的容器不会创建自己的网卡和配置自己的IP，而是和一个指定的容器共享IP、端口范围等。指定的容器关闭后，新创建的容器网络模式就变成了 none 模式。

`自定义网络`: nginx-tomcat-net-2，加入该网络的 容器，可以通过 容器名+端口访问。自定义网络之后，不同子网的服务则不可以互相通信。
...
# 匿名挂载：只指定 容器目录，Docker 会帮你自动创建。(会将容器内部自带的文件存储在默认的存放路径中(绑定挂载没有这个功能) /var/lib/docker/volumes/8b3ac6b427613ce1954a427aeea8052cfaf928e625dcabe236fce1380c325995/_data) (Mounts.Name: 8b3ac6b427613ce1954a427aeea8052cfaf928e625dcabe236fce1380c325995)
...
**WORKDIR:** 声明镜像的默认工作目录 (指定再创建容器后，终端默认登陆的进来工作目录，一个落脚点。)

**ADD:**

**CMD:** 需要执行的命令 (在 WORKDIR 下执行的，CMD 可以写多个，但是只以最后一个为准。) (可以是 shell 格式，也可以是 exec 格式)
...
**EXPOSE:** 只是做一个提示，即使不写对镜像的功能也没有影响。

**ENTRYPOINT** 类似于 CMD 指令，但是其不会被 docker run 后面的命令覆盖，而且这些命令参数会被当做参数发送给 ENTRYPOINT 指令指定的程序。与 CMD 二选一。如果镜像中 ENTRYPOINT 和 CMD 都存在，则 CMD 将作为 ENTRYPOINT 的参数使用，最终运行的是 CMD。

**USER** 指定该镜像以什么样的用户去执行，如果不指定，默认是 root (一般不配置)

```shell
# FROM image_name: tag 定义了使用哪个基础镜像启动构建流程
# LABEL author=user_name 声明镜像的创建者
# ENV key value 构建过程中设置环境变量 (可以写多条) (可以在后续的 任何 RUN 指令中使用，就如同在命令前面指定了环境变量前缀一样。)
# RUN command 是 Dockerfile 的核心部分 (可以写多条) (command: shell 命令，执行命令行指令。)
...
# CMD ["npm", "run", "start"] 指定容器启动后要做的事情。可以在启动容器时被替换和覆盖。(会被 docker run 之后的参数替换)
# ENTRYPOINT 指定容器启动时执行的默认程序，一般运行容器时不会被替换或覆盖。除非使用 --entrypoint 进行指定。

# RUN git clone https://github.com/xxx 可以从 git 上拉取代码

# COM 和 RUN 的区别：RUN 是在 docker build 时运行，CMD 是在 docker run 时运行。
# 一般是变参才会用 CMD。
```
...
可以通过 Docker-compose 批量编写参数，可以批量的管理容器。解决了容器与容器之间如何管理编排的问题。
